...
<template>
  <div class="w-full h-full min-h-screen bg-white">
    <!-- Calendar Header -->
    <div class="p-4 border-b flex items-center justify-between">
      <div class="flex items-center space-x-0.5 w-full justify-between">
        <div class="flex items-center space-x-0.5">
          <button
            class="py-3 px-4 rounded-l-lg hover:bg-gray-100 bg-[#F2F4F7]"
            @click="changeWeek(-1)"
          ...
          <span
            class="text-lg font-medium bg-[#F2F4F7] py-3 px-6 text-sm"
          >{{ currentMonth }} {{ currentYear }}</span>
          <button
            class="py-4 rounded-r-lg px-4 hover:bg-gray-100 bg-[#F2F4F7]"
            @click="changeWeek(1)"
    ...
    <div class="flex border-b">
      <div class="w-20 border-r bg-white"></div>
      <!-- Added overflow-x-auto wrapper and min-w-max grid -->
      <div class="flex-1 overflow-x-auto">
        <div class="grid grid-cols-7 min-w-max">
          <div
            v-for="day in visibleDays"
            :key="day.fullDate"
            class="p-2 text-center border-r min-w-[150px]"
          >
            <div class="text-sm text-gray-600">{{ day.name }}</div>
            <div class="text-2xl font-medium">{{ day.date }}</div>
          </div>
    ...
    <div class="flex flex-1">
      ...
      <!-- Calendar Grid -->
      <!-- Wrapped grid with overflow-x-auto and ensured grid cells have a minimum width -->
      <div class="flex-1 overflow-x-auto">
        <div class="grid grid-cols-7 min-w-max">
          <div
            v-for="day in visibleDays"
            :key="day.fullDate"
            class="border-r relative min-w-[150px]"
            :class="{ 'bg-blue-50': hasEventsOnDay(day) }"
            @mouseover="showDayTooltip(day, $event)"
            @mouseleave="hideTooltip"
          >
...
<style scoped>
/* Optional: Adjust grid height if needed */
.calendar-grid {
  height: calc(100vh - 120px);
}
</style>
