        ...
        >
          ...
          <div class="flex items-center w-full">
            ...
            <div class="w-full space-y-1 pl-3">
             ...
             <div class='flex justify-between items-center w-full'>
              <p class="text-sm text-[#667185] truncate max-w-[200px]">{{ formatMessagePreview(user?.lastMessage?.content) }}</p>
              <svg v-if="user?.unreadMessagesCount > 0" width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="5" cy="5" r="4" fill="#099137"/>
              </svg>
             </div>
  ...
  function formatMessagePreview(content: any) {
    ...
    try {
      ...
      if (parsed.type === 'image') {
        if (parsed.file?.name) {
          return `🖼️ ${parsed.file.name}`;
        }
        return '🖼️ Image shared';
      }
      
      // If it's JSON but not a file/image object, treat as text
