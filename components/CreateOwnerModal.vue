<!-- components/CreateOwnerModal.vue -->
<template>
  <div class="fixed inset-0 z-50 overflow-hidden bg-white flex flex-col">
    <!-- Header -->
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
      <button @click="$emit('close')" class="flex items-center text-gray-500 hover:text-gray-700">
        ...
        <div class="flex items-center w-full">
          ...
          <div class="flex flex-col items-center flex-1">
            <div :class="[
              ...
              <div v-else :class="currentStep === 1 ? 'bg-white w-3 h-3 rounded-full' : ''"></div>
            </div>
            <span :class="[
          ...
          <div class="flex flex-col items-center flex-1">
            <div :class="[
              ...
              <div v-else :class="currentStep === 2 ? 'bg-white w-3 h-3 rounded-full' : ''"></div>
            </div>
            <span :class="[
            ...
            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6">
              <div v-if="!uploadedFile" class="text-center">
                ...
                <div class="mt-4">
                  ...
                  <p class="text-xs text-gray-500 mt-1">JPEG, PNG, PDF, and MP4 formats, up to 50 MB.</p>
                </div>
                <button
                  type="button"
              ...
              <div v-else class="flex items-center justify-between">
                <div class="flex items-center">
                  <div class="w-10 h-10 bg-purple-100 rounded flex items-center justify-center">
                    <span class="text-purple-600 font-medium text-xs">JPG</span>
                  </div>
                  <div class="ml-3">
                    <p class="text-sm font-medium text-[#0C111D]">{{ uploadedFile.name }}</p>
                    <p class="text-sm text-gray-500">{{ uploadedFile.size }} - {{ uploadProgress }}% Uploaded</p>
                  </div>
                </div>
                <div class="w-6 h-6">
                  <svg v-if="uploadProgress < 100" class="animate-spin text-blue-600" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                  </svg>
                  <svg v-else class="text-green-600" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                  </svg>
                </div>
              </div>
    ...
    <div class="flex justify-between items-center px-6 py-4 border-t border-gray-200 bg-white">
      ...
      <button
        ...
        class="px-6 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center"
      >
        Continue
      ...
      <button
        ...
        @click="publishOwner"
        :disabled="!canPublish"
        class="px-6 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
      >
        Publish
      </button>
    </div>
...
const canPublish = computed(() => {
  if (formData.ownerType === 'Individual') {
    return formData.ninNumber && uploadedFile.value && uploadProgress.value === 100
  } else if (formData.ownerType === 'Business') {
    return formData.cacNumber && uploadedFile.value && uploadProgress.value === 100
  }
  return false
})
